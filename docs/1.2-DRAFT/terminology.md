...
_Data Entity_: A directory, file or other resource that is considered _contained_ by the _RO-Crate_ and described in its _RO-Crate Metadata File_.
...
_JSON-LD_: A JSON-based file format for storing _Linked Data_. This document assumes [JSON-LD 1.0]. JSON-LD map JSON keys to _URIs_ using for instance the _RO-Crate JSON-LD Context_.
...
_RO-Crate JSON-LD Context_: A JSON-LD [context][JSON-LD context] that provides Linked Data mapping for RO-Crate metadata to vocabularies like [Schema.org]. This mapping assigns meaning to the JSON keys.
