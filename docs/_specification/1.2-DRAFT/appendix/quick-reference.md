...
| nested entities | MUST NOT | Must NOT be present - instead describe these as separate contextual entities in the flat `@graph` list | MUST | as the JSON-LD must be [flattened] |
...
| [license] | SHOULD | Should link to a _Contextual Entity_ or _Data Entity_ in the _RO-Crate Metadata Document_ with a name and description | see [contextual entities for licenses](#TODO)
| [license] | MAY | May be a textual description of how the RO-Crate may be used | 
| [identifier] | SHOULD | Should be used to indicate the _persistent identifier_ for the crate, if one exists
| [identifier] | SHOULD | Should use the approach set out in the [Science On Schema.org guides] to reference a contextual entity for the identifier | see [contextual entities for identifiers](#TODO)
...
| [identifier] | SHOULD | Resolving the `identifier` programmatically should return the _RO-Crate Metadata Document_ or an archive (e.g. ZIP) that contains the _RO-Crate Metadata File_, using [content negotiation](data-entities#retrieving-an-ro-crate) and/or [Signposting]
...
| ------- | ------| ------ | ------- | ------- |
| Any | `@id` | MUST | If the entity represents a file or directory present in the _RO-Crate Root_, `@id` must be a relative URI | |
| Any | `@id` | MUST | If `@id` is a relative URI, the file or directory represented by the entity must be present in the _RO-Crate Root_ at that relative path | |
| Any | `@id` | MUST | If in a [Detached RO-Crate](structure#detached-ro-crate-package), `@id` must be an absolute URI | |
...
| Any | `name` | SHOULD | Should be present | |
| Any | `description` | SHOULD | Should be present | |
| Any | `license` | SHOULD | If different to the `license` on the _Root Data Entity_, should reference a [CreativeWork] entity | [source](contextual-entities#licensing-access-control-and-copyright)
| Any | `contentLocation` / `spatialCoverage` | SHOULD | One of these properties should be present if the entity is associated with a geographical location or region | [source](contextual-entities#places)
...
| Any | `citation` | MUST | If associating a publication with a dataset, `citation` must include a URL (for example a DOI URL) as the `@id` of a publication | this feels a weird requirement... [source](contextual-entities#publications-via-citation-property)
...
| Property/Target | Severity | Description | Eli attention |
| ------- | ------| ------ | ------- |
| All | MUST | Must follow requirements for [Directory Data Entities](#data-entities)  | |
...
| Person | `@id` | SHOULD | Should be an ORCID URI, if possible | implicit! | [source](../metadata#recommended-identifiers) | 
...
| Organization | `@id` | SHOULD | Should be an ROR URI, if possible | [source](../metadata#recommended-identifiers)
...
| Place | `@id` | SHOULD | Should be a [geonames] URL, e.g. `http://sws.geonames.org/8152662/` | [source](../metadata#recommended-identifiers) | 
